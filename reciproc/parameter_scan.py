...
from .threshold_network import ei_network
...
def parameter_scan(par_list, mesh_list, fixed_par, nb_nodes, netfunc, tstep=20):
    mesh1 = mesh_list[0]
    mesh2 = mesh_list[1]
    results = np.zeros((len(mesh1), len(mesh2), nb_nodes))
...
def run_network_avalanche_size(nb_nodes=10, p_edge=0.1, p_exc=0.5,
                               p_reci=0, netfunc=ei_network, tstep=20):
    ...
    ava_size_list = network_avalanche_size(graph, netfunc, tstep=tstep)
